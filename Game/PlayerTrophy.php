	...
	public function check(string $name): bool {
		...
		switch ($name) {
			...
			case 'not-found-shots':
				...
				return $this->checked[$name];
			case 'team-50':
				$this->checked[$name] = !$this->solo && $this->player->score > 0 && $this->player->team?->score !== 0 && $this->player->team?->playerCount > 1 && ($this->player->score / $this->player->team->score) > 0.45;
				return $this->checked[$name];
			case 'kd-1':
			...
			case 'favouriteTarget':
				/** @var Player|null $favouriteTarget */
				$favouriteTarget = $this->player->favouriteTarget;
				$this->checked[$name] = isset($favouriteTarget)
					&& $this->player->hits !== 0
					&& $this->player->getHitsPlayer($favouriteTarget) / $this->player->hits > 0.45;
				return $this->checked[$name];
			case 'favouriteTargetOf':
				...
				$this->checked[$name] = isset($favouriteTarget)
					...
					&& $favouriteTarget->getHitsPlayer($this->player) / $this->player->deaths > 0.45;
				return $this->checked[$name];
			case 'fair':
				$maxDelta = 0;
				$hits = [];
