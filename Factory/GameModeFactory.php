...
class GameModeFactory implements FactoryInterface
{
     ...
     */
    public static function findModeObject(string $system, ?Row $mode, GameModeType $modeType) : AbstractMode {
        ...
        if (!class_exists($class)) {
            throw new GameModeNotFoundException(
              'Cannot find game mode class: '.(isset($dbName) ? $classBase.$classSystem.$classNamespace.$dbName.'|' :
                '').$classBase.$classSystem.$classNamespace.$className.'|'.$classBase.$classNamespace.$className
            );
        }
    ...
    public static function find(
      ...
      string       $system = ''
        ...
        /** @var Row|null $mode */
        $mode = DB::select('vModesNames', 'id_mode, name, system')
                  ->where(
